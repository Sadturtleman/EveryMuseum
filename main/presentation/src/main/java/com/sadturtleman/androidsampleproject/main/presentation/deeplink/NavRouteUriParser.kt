package com.sadturtleman.androidsampleproject.main.presentation.deeplink

import android.net.Uri
import android.util.Log
import androidx.navigation3.runtime.NavKey
import com.sadturtleman.androidsampleproject.common.domain.navigation.NavRoute
import com.sadturtleman.androidsampleproject.main.presentation.navigation.GenericNavKey
import com.sadturtleman.androidsampleproject.main.presentation.navigation.appRouteByPath
...
fun Uri.toNavRoute(): NavRoute {
    val segments = pathSegments?.takeIf { it.isNotEmpty() } ?: return NavRoute(SearchPage.PATH)
 ...
 * - URI 가 없거나 미등록 path 면 Search 단일 스택으로 fallback.
 * - 등록된 path 면 해당 [com.sadturtleman.androidsampleproject.main.presentation.navigation.AppRoute] 의
 *   syntheticStack 을 그대로 사용한다.
 */
fun resolveStartStack(uri: Uri?): List<NavKey> {
    if (uri == null) return listOf(GenericNavKey(SearchPage.PATH))
    val route = uri.toNavRoute()
    val appRoute = appRouteByPath[route.path]
    if (appRoute == null) {
        Log.w(TAG, "No matching path for uri=$uri (path=${route.path})")
        return listOf(GenericNavKey(SearchPage.PATH))
    }
